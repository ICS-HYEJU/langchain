...
DIAGRAMS = [
    Diagram(
        ...
        description=(
            "금오공과대학교 학사행정 매뉴얼 PDF 등 학사행정 문서를 검색하고, 문서가 관련되어 있더라도 "
            "질문에 답하기에 충분하지 않거나 학과별 졸업학점처럼 웹 공개 최신 정보가 필요한 질문이면 "
            "금오공과대학교 도메인 웹검색을 보조 근거로 검토하는 흐름입니다."
        ),
...
""",
        flow=[
            ...
            "`web_search`: 관련 문서가 재검색 후에도 부족하거나, 관련 문서는 있지만 답변 충분성이 낮으면 `site:kumoh.ac.kr` 조건으로 검색합니다. DuckDuckGo가 차단되면 Bing 검색, 금오공과대학교 후보 페이지 직접 조회 순서로 보완합니다. 도메인은 `ACADEMIC_RAG_WEB_SEARCH_DOMAIN`으로 바꿀 수 있습니다.",
