...
DIAGRAMS = [
    Diagram(
        ...
        description=(
            ...
            "질문에 답하기에 충분하지 않으면 KHU 도메인 웹검색을 보조 근거로 검토하는 흐름입니다."
        ),
...
""",
        flow=[
            ...
            "`assess_context_sufficiency`: 관련 문서가 있어도 그 문서만으로 구체적인 답변이 가능한지 다시 평가합니다. 현재 공지, 일정, URL, 양식처럼 변경 가능성이 큰 질문은 내부 문서에 명확한 근거가 없으면 부족하다고 판단합니다.",
            "`web_search`: 관련 문서가 재검색 후에도 부족하거나, 관련 문서는 있지만 답변 충분성이 낮으면 `site:khu.ac.kr` 조건으로 웹검색을 수행합니다. 도메인은 `ACADEMIC_RAG_WEB_SEARCH_DOMAIN`으로 바꿀 수 있습니다.",
